...
def test_freeable_cache_exact_parity(fixture_tree, tmp_path):
    """The v4 cache exposes explicit dimensions instead of v3 oracle parity."""
    import sqlite3
    ...
    db = tmp_path / "v4.db"
    env = {"DUH_DB": str(db), "PATH": "/usr/bin:/bin"}
    subprocess.run([str(rs_bin), "scan", str(fixture_tree)], env=env, check=True, capture_output=True)
    subprocess.run([str(rs_bin), "freeable", str(fixture_tree)], env=env, check=True, capture_output=True)
    con = sqlite3.connect(db)
    columns = {r[1] for r in con.execute("PRAGMA table_info(freeable_cache)")}
    assert {"guaranteed", "conditional_shared", "uncertain",
            "locked_guaranteed_here", "locked_conditional_here"} <= columns
    rows = con.execute(
        "SELECT freeable, guaranteed, locked_here, locked_guaranteed_here "
        "FROM freeable_cache").fetchall()
    assert rows and all(r[0] == r[1] and r[2] == r[3] for r in rows)
